...
object Data {
    ...
    /**
     * Contains three documents, The first and the second are valid The 3d has expired validity info
     */
    val ThreeDocumentVP =
        """
    ...
    val caCerts: NonEmptyList<X509Certificate> by lazy {
        DefaultResourceLoader().getResource("classpath:trusted-issuers.jks").inputStream.use {
                inputStream ->
            val keyStore = loadKeystore(inputStream)
            val certs = X5CShouldBe.trustedCAs(keyStore).toNonEmptyListOrNull()
            requireNotNull(certs) {
                "Unable to load X509 Certificates from 'classpath:trusted-issuers.jks'"
            }
        }
    }
...
class DeviceResponseValidatorTest {
    ...
    fun `a vp_token having a single document skipping chain validation should be valid`() {
        val validDocuments = run {
            val docV = DocumentValidator(x5CShouldBe = X5CShouldBe.Ignored, clock = clock)
            val vpValidator = DeviceResponseValidator(docV)
...
private fun deviceResponseValidator(
    caCerts: NonEmptyList<X509Certificate>,
    clock: Clock,
